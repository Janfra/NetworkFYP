...
void ANetworkFYPController::BeginPlay()
{
    ...
    if (GetNetMode() != ENetMode::NM_DedicatedServer) 
    {
		...
		UNetworkFYPOnlineServiceSubsystem* OnlineSubsystem = UNetworkFYPOnlineServiceSubsystem::TryGetSubsystem(this);
		if (!OnlineSubsystem) 
		{
			return;
		}
		...
		if (LocalPlayer) 
		{
			FPlatformUserId LocalPlayerPlaformUserId = LocalPlayer->GetPlatformUserId();
			UE_LOG(LogNetworkFYPOnlineServiceSubsystem, Log, TEXT("Registering PlatformUserId: %d"), LocalPlayerPlaformUserId.GetInternalId());

			// Attempt to log in this player and get callback once successful
			OnlineSubsystem->RegisterLocalOnlineUser(LocalPlayerPlaformUserId, FLoggedInCallback::CreateUObject(this, &ThisClass::JoinSession));
		}
    }
}
...
void ANetworkFYPController::JoinSession()
{
	...
	{
		FPlatformUserId LocalPlayerPlaformUserId = LocalPlayer->GetPlatformUserId();
		OnlineSubsystem->FindSessions(LocalPlayerPlaformUserId);
	}
}
